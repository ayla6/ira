...
pub fn run_schema_migrations(conn: &Connection) {
    ...
    ensure_column(conn, "game_variants", "count_playtime", "INTEGER NOT NULL DEFAULT 1");
    ensure_column(conn, "game_variants", "logo_position", "TEXT NOT NULL DEFAULT 'bottom-left'");
    ensure_column(conn, "game_variants", "logo_size", "INTEGER NOT NULL DEFAULT 50");
