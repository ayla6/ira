...
fn main() {
    ...
    let feed = |engine: &mut MappingEngine, pad: &mut PadState, label: &str, source, value| {
        let outputs = engine.process(InputEvent { source, value, timestamp_us: 0 });
        for out in &outputs {
            println!("  {label} -> {out:?}");
    ...
    feed(&mut engine, &mut pad, "button L press", InputSource::Button(GamepadButton::LeftTrigger), 1.0);
    feed(&mut engine, &mut pad, "button L release", InputSource::Button(GamepadButton::LeftTrigger), 0.0);
    for v in [0.0f32, 0.5, 1.0, 0.0] {
        feed(&mut engine, &mut pad, &format!("axis L {v}"), InputSource::Axis(GamepadAxis::LeftTrigger), v);
    }
    println!("final pad: l2={} r2={} state2-bit0={}", pad.l2, pad.r2, pad.l2 > 0.0);
}
