...
pub fn show_edit_game_dialog(state: &SharedState, db_id: i64) {
    ...
    if (emu_trophy_source == ira_models::GSE || emu_trophy_source == ira_models::NGE) && !emu_exe.is_empty() {
        ...
        if is_installed {
            ...
            let versions = if emu_trophy_source == ira_models::GSE {
                ira_platforms::api_emulators::list_gse_versions(&emu_save_dir)
            } else {
                ira_platforms::api_emulators::list_gog_versions(&emu_save_dir)
            };
    ...
    // --- Variants page ---
    let variants: Vec<ira_models::GameVariant> = ira_db::get_variants(&state.borrow().db, db_id).unwrap_or_default();
    ...
    add_btn.set_margin_top(8);
    {
        ...
        let new_v = ira_models::GameVariant { game_id: db_id, ..Default::default() };
