...
const COLOR_HEX: &[&str] = &["#3584e4","#33d17a","#ff7800","#e01b24","#9141ac","#f5c211"];
const OTHER_HEX: &str = "#888a8a";

pub(super) fn color_hex(i: usize) -> &'static str { COLOR_HEX[i.min(COLOR_HEX.len()-1)] }
...
pub(super) fn assign_game_colors(s: &[ira_models::PlaySession]) -> GameColorAssignment {
    ...
    for s in s { *t.entry(s.game_id).or_default() += s.duration_seconds as f64; }
    ...
    GameColorAssignment { top_games: top, color_map: map }
}
...
struct State {
    weeks: Vec<WeekData>, current_idx: usize, selected_day: usize, is_single_game: bool,
    chart: BarChart,
...
pub(super) fn build_weekly_chart(weeks: Vec<WeekData>, is_single_game: bool) -> gtk4::Widget {
    ...
    row.set_hexpand(true); row.set_vexpand(true);
    ...
    { let st = state.clone(); pw.connect_clicked(move |_| { let mut s = st.borrow_mut(); if s.current_idx > 0 { s.current_idx -= 1; drop(s); rebuild(&st, None); } }); }
    { let st = state.clone(); nw.connect_clicked(move |_| { let mut s = st.borrow_mut(); if s.current_idx + 1 < s.weeks.len() { s.current_idx += 1; drop(s); rebuild(&st, None); } }); }
    { let st = state.clone(); pd.connect_clicked(move |_| nav_day(&st, -1)); }
    { let st = state.clone(); nd.connect_clicked(move |_| nav_day(&st, 1)); }
...
fn nav_day(st: &Rc<RefCell<State>>, dir: i32) {
    let (wk, dy) = { let s = st.borrow();
        let nd = s.selected_day as i32 + dir;
        if nd < 0 { if s.current_idx > 0 { (s.current_idx - 1, 6usize) } else { return; } }
        else if nd > 6 { if s.current_idx + 1 < s.weeks.len() { (s.current_idx + 1, 0usize) } else { return; } }
        else { (s.current_idx, nd as usize) }
    };
...
fn nav_week(st: &Rc<RefCell<State>>, dir: i32) {
    ...
    if ni >= 0 && (ni as usize) < s.weeks.len() {
        s.current_idx = ni as usize; drop(s);
        rebuild(st, None);
    }
}

fn rebuild(state: &Rc<RefCell<State>>, forced: Option<usize>) {
    let (week, is_sg, today) = { let s = state.borrow();
        (s.weeks[s.current_idx].clone(), s.is_single_game, chrono::Local::now().date_naive()) };
    let max_v = week.days.iter().map(|d| d.total).fold(1.0, f64::max);
    let (interval, nmax) = axis_config(max_v);

    let sel = forced.unwrap_or_else(|| week.days.iter().position(|d| d.date == today)
...
fn update_day_btns(s: &State) {
    s.prev_d.set_sensitive(s.current_idx > 0 || s.selected_day > 0);
    s.next_d.set_sensitive(s.current_idx + 1 < s.weeks.len() || s.selected_day < 6);
}

fn select_day(state: &Rc<RefCell<State>>, idx: usize) {
    let is_sg = { let mut s = state.borrow_mut(); s.selected_day = idx; s.is_single_game };
    { let s = state.borrow();
...
fn build_nav() -> (gtk4::Box, gtk4::Button, gtk4::Button, gtk4::Button, gtk4::Button) {
...
fn update_stats(s: &State) {
    ...
    let wt = if week.week_start <= today && today <= we {
        ...
        format!("{} \u{2013} {} {} {}", week.week_start.format("%b %-d"), we.format("%b %-d"), dash, format_duration(week.week_total as i64))
    };
    s.week_label.set_markup(&wt);

    let ht = if day.total > 0.0 {
        format!("{} {} {}", day.date.format("%a, %b %d"), dash, format_duration(day.total as i64))
    } else { day.date.format("%a, %b %d").to_string() };
...
mod tests {
    ...
    #[test] fn test_color_hex() { assert_eq!(color_hex(0), "#3584e4"); assert_eq!(color_hex(10), color_hex(5)); }

    #[test] fn test_axis_config() {
    ...
    #[test] fn test_assign_colors() {
        let s: Vec<PlaySession> = (1..=8i64).map(|i| PlaySession { id:i, game_id:i, variant_id:None, started_at:1000+i, ended_at:1000+i+100*(9-i), duration_seconds:100*(9-i) }).collect();
        let a = assign_game_colors(&s);
        assert_eq!(a.top_games.len(), 5); assert!(!a.color_map.contains_key(&8));
    }
}
