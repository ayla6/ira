...
pub unsafe extern "system" fn queue_present(
    ...
    let sc_data = {
        ...
        map.as_ref().and_then(|m| m.get(&(swapchain.as_raw()))).cloned()
    };
    ...
    let wait_stages = vec![vk::PipelineStageFlags::COLOR_ATTACHMENT_OUTPUT; wait_sem_count as usize];
...
unsafe fn chain_present(queue: vk::Queue, present_info: &vk::PresentInfoKHR) -> vk::Result {
    let fns = {
        ...
        map.as_ref().and_then(|m| m.values().next()).and_then(|d| d.fns)
    };
    if let Some(fns) = fns {
