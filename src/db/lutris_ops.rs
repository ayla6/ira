...
pub fn upsert_matching(conn: &DbConn, lutris_db_id: i64, steam_id: &str, kind: &str, platform_id: &str) -> Result<i64, String> {
    ...
    let existing_by_lutris: Option<i64> = tx.query_row(
    ...
    if let Ok(h) = tx.query_row::<bool, _, _>(
        ...
        let _ = tx.execute("UPDATE games SET hidden = ?1 WHERE id = ?2", params![h, new_id]);
        let _ = tx.execute("DELETE FROM lutris_meta WHERE lutris_id = ?1", params![lutris_db_id]);
    }
    tx.commit().map_err(|e| e.to_string())?;
    Ok(new_id)
}
