...
fn build_shadps4_games(db: &db::DbConn, save_dir: &str) -> Vec<Game> {
    ...
    for shad in &shad_games {
        ...
        let (db_id, title, hidden, logo_position, logo_size, sort_title, sgdb_id, shadps4_version) = match entry {
            Some(e) => (e.id, e.title, e.hidden, e.logo_position, e.logo_size, e.sort_title, e.sgdb_id.clone().unwrap_or_default(), e.shadps4_version.clone().unwrap_or_default()),
            None => {
                // Insert new entry
                match db::add_game(db, "ps4", &shad.npwr_id, &shad.serial, &shad.title) {
                    Ok(id) => (id, shad.title.clone(), false, "bottom-left".to_string(), 50, String::new(), String::new(), String::new()),
                    Err(e) => {
