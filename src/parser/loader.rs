use crate::db::DbConn;
use crate::models::{AchievementStatus, Game, GameEntry, MergedAchievement};
...
pub fn load_game(entry: &GameEntry, save_dir: &str) -> Result<Game, String> {
    ...
    let ach_dir = super::paths::achievements_dir(save_dir, app_id);
    ...
    let image_dir = if kind == crate::models::PS4 {
        super::paths::ps4_data_dir(save_dir, app_id)
    } else if crate::models::has_steam_enrichment(&entry.trophy_source) {
        super::paths::data_dir(save_dir, app_id)
    } else if entry.sgdb_id.as_deref().filter(|s| !s.is_empty()).is_some() {
        super::paths::sgdb_data_dir(save_dir, entry.sgdb_id.as_deref().unwrap())
    } else {
        super::paths::data_dir(save_dir, app_id)
    };
    ...
    if icon_png.is_file() {
        ...
        let ra_icon = super::paths::ra_icon_path(save_dir, app_id);
        if ra_icon.is_file() {
            game.icon_path = ra_icon.to_string_lossy().into_owned();
        }
        let icon_ico = image_dir.join("icon.ico");
        if icon_ico.is_file() {
            match super::icons::convert_ico_to_png(&icon_ico) {
    ...
    let status_map = if is_steam_native {
        ...
        let status_path = super::paths::unlock_status_path(save_dir, &entry.trophy_source, app_id, platform_id);
        super::status::load_status_map(&status_path)
    };
    ...
    if !has_meta && !steam_native_data.achievements.is_empty() {
        ...
        for meta in meta_list {
            ...
            game.achievements.push(MergedAchievement {
                ...
                icon_path: super::icons::find_icon_path(&ach_dir, &meta.icon),
                icon_gray_path: super::icons::find_icon_path(&ach_dir, &icon_gray),
                global_percent: 0.0,
                trophy_type: '\0',
            });
        }
