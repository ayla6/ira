...
pub fn unlock_status_path(save_dir: &str, trophy_source: &str, app_id: &str, platform_id: &str) -> PathBuf {
    match trophy_source {
        crate::models::NGE => Path::new(save_dir).join("gog").join(GALAXY_ID).join(platform_id).join("achievements.json"),
