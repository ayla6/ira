...
pub fn get_app_types(app_ids: &[u32]) -> HashMap<u32, String> {
    ...
    let Some(path) = appinfo_path() else { return result; };
    let Ok(data) = std::fs::read(&path) else { return result; };
    if data.len() < 24 { return result; }

    let st_offset = u32::from_le_bytes([data[8], data[9], data[10], data[11]]) as usize;
    if st_offset + 4 > data.len() { return result; }

    let Some(type_idx) = find_string_index(&data, st_offset, "type") else { return result; };
...
pub fn get_clienticon(app_id: u32) -> Option<String> {
    ...
    let st_offset = u32::from_le_bytes([data[8], data[9], data[10], data[11]]) as usize;
    if st_offset + 4 > data.len() { return None; }
...
fn find_string_index(data: &[u8], table_offset: usize, target: &str) -> Option<usize> {
    if table_offset + 4 > data.len() { return None; }
    let size = u32::from_le_bytes([
        data[table_offset], data[table_offset+1], data[table_offset+2], data[table_offset+3]
    ]) as usize;
